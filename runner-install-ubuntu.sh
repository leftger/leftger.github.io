#!/usr/bin/env bash
# ==============================================================================
# Gerzain's Ubuntu GitHub Actions Self-Hosted Runner Installer
#
# Turns a bare Ubuntu LTS machine into a hardened, integration-test-oriented
...
#
# Or locally:
#   sudo ./runner-install-ubuntu.sh --url https://github.com/OWNER/REPO --token <TOKEN>
#
# Summary of automated actions (each section can be skipped or run alone):
#   1.  base        Ubuntu base packages, APT tuning, actions/runner runtime libs
...
#   6.  containers  Rootless Podman, docker CLI shim, AppArmor userns profiles
#   7.  security    ufw, sysctl hardening, AppArmor, unattended-upgrades
...
#   * Ubuntu 23.10+ denies unprivileged user namespace creation through AppArmor.
#     Targeted profiles granting 'userns,' to Podman and its helpers are installed
#     so rootless containers work without disabling that kernel restriction.
# ==============================================================================
...
SCRIPT_NAME="runner-install-ubuntu.sh"
...
# confdef/confold keep existing conffiles on upgrade, so a re-run never prompts
# or silently replaces configuration. DPkg::Lock::Timeout waits out a lock held
# by unattended-upgrades or apt-daily instead of failing the whole run.
APT_FLAGS=(
    "-y"
...
# Print only the requested packages that actually exist in the enabled APT
# sources, so one renamed or absent package cannot fail the whole transaction.
# Ubuntu moves packages between main and universe across releases, so this keeps
# the script usable across a mixed fleet.
apt_filter_available() {
    ...
    if ! command -v apt-cache >/dev/null 2>&1; then
        log_warn "apt-cache unavailable; skipping package availability filtering."
        printf '%s\n' "${wanted[@]}"
        return 0
    fi
    ...
    if [ "${#missing[@]}" -gt 0 ]; then
        log_warn "Not in enabled APT sources, skipping: ${missing[*]}"
    fi
...
apt_install() {
    ...
    run_sudo apt-get install "${APT_FLAGS[@]}" "${pkgs[@]}"
}

# Most of the interesting packages here (podman, sccache, uv, qemu-system-arm,
# gcc-arm-none-eabi) live in 'universe', which is disabled on some minimal and
# server images. Probing for a universe-only package avoids rewriting APT sources
# on systems that already have it enabled.
apt_ensure_universe() {
    if [ "$DRY_RUN" -eq 1 ]; then
        log_info "[DRY-RUN] Would ensure the 'universe' APT component is enabled."
        return 0
    fi

    if apt-cache show podman >/dev/null 2>&1; then
        log_info "APT component 'universe' is available."
        return 0
    fi

    log_info "Enabling the 'universe' APT component..."
    if ! command -v add-apt-repository >/dev/null 2>&1; then
        apt_install software-properties-common
    fi

    if command -v add-apt-repository >/dev/null 2>&1; then
        run_sudo add-apt-repository -y universe >/dev/null 2>&1 || \
            log_warn "Could not enable 'universe'; some packages may be skipped."
        apt_refresh
    else
        log_warn "add-apt-repository is unavailable; enable 'universe' manually if packages are missing."
    fi
}
...
# userns_create to unconfined processes. That breaks rootless Podman, crun,
# buildah, and the helpers they spawn (slirp4netns, pasta, fuse-overlayfs), so
# container-based integration tests fail with EPERM.
#
# The targeted fix is a per-binary profile granting 'userns,', which leaves the
# kernel restriction in place for everything else. --relax-userns sets the sysctl
# instead, for suites that create user namespaces from binaries that cannot be
# enumerated ahead of time (bubblewrap, kind, nsjail, fakeroot).
# ------------------------------------------------------------------------------
userns_restricted() {
    local flag=/proc/sys/kernel/apparmor_restrict_unprivileged_userns

    if [ ! -e "$flag" ]; then
        return 1
    fi
    [ "$(cat "$flag" 2>/dev/null || echo 0)" = "1" ]
}

apparmor_userns_profiles() {
    local bin=""
    local path=""
    local found=0
    local loaded=0
    ...
    if ! userns_restricted; then
        log_info "Kernel does not restrict unprivileged user namespaces; no AppArmor profile needed."
        return 0
    fi

    if [ "$RELAX_USERNS" -eq 1 ]; then
        log_info "--relax-userns set; per-binary AppArmor profiles are unnecessary."
        return 0
    fi

    if ! command -v apparmor_parser >/dev/null 2>&1; then
        log_warn "apparmor_parser is missing; rootless containers may fail with EPERM."
        log_warn "Re-run with --relax-userns to lift the restriction instead."
        return 0
    fi

    # A profile on a parent does not extend to processes it execs, so every
    # binary that creates a user namespace needs its own entry.
    for bin in podman crun buildah conmon slirp4netns pasta fuse-overlayfs rootlessport; do
        ...
        write_heredoc "/etc/apparmor.d/${bin}" 0644 root:root <<EOF
# Managed by runner-install-ubuntu.sh - grants unprivileged user namespace
# creation to ${bin} (${path}) on Ubuntu 23.10 and newer.
abi <abi/4.0>,
include <tunables/global>

profile ${bin} ${path} flags=(unconfined) {
    userns,

    include if exists <local/${bin}>
}
EOF

        if run_sudo apparmor_parser -r "/etc/apparmor.d/${bin}"; then
            loaded=$((loaded + 1))
        else
            log_warn "Could not load the AppArmor profile for ${bin}."
        fi
    done


    if [ "$found" -eq 0 ]; then
        # Expected under --dry-run, where the container runtime is not installed yet.
        log_info "No user-namespace binaries on PATH to profile yet."
    elif [ "$loaded" -gt 0 ]; then
        log_info "Installed ${loaded} AppArmor profile(s) granting 'userns' for rootless containers."
    else
        log_warn "No AppArmor profiles could be loaded; rootless containers may fail."
    fi
}
...
print_help() {
    cat <<EOF
Gerzain's Ubuntu GitHub Actions Runner Installer v${RUNNER_INSTALL_VERSION}

Usage:
  ./runner-install-ubuntu.sh [OPTIONS]
  curl -fsSL https://leftger.github.io/runner-install-ubuntu.sh | sudo bash -s -- [OPTIONS]
...
Behaviour:
      ...
      --no-autoupdate       Skip the unattended-upgrades security-update timer
      ...
      --relax-userns        Set kernel.apparmor_restrict_unprivileged_userns=0 so
                            any binary may create a user namespace. Needed by
                            bubblewrap/kind/nsjail-style suites that cannot be
                            covered by the default per-binary AppArmor profiles.
                            This removes a kernel mitigation for the whole host.
...
Sections (combine --*-only flags to run a subset):
      --skip-base           Skip Ubuntu base packages and runner runtime libraries
      ...
      --skip-security       Skip ufw, sysctl, AppArmor, and unattended-upgrades setup
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --base-only | --rust-only | --python-only | --arm-only | --qemu-only | \
            ...
            ;;
        -v | --version)
            printf 'runner-install-ubuntu.sh %s\n' "$RUNNER_INSTALL_VERSION"
            exit 0
            ;;
        -h | --help)
            print_help
...
if [ ! -r /etc/os-release ]; then
    log_error "Cannot read /etc/os-release; this installer only supports Ubuntu."
    exit 1
fi
...
# Ubuntu Core and other image-based variants have no writable dpkg database.
if [ ! -d /var/lib/dpkg ]; then
    log_error "No dpkg database found. This installer needs a traditional Ubuntu install."
    log_error "Ubuntu Core and other image-based variants are not supported."
    exit 1
fi

case "$OS_ID" in
    ubuntu) ;;
    debian | linuxmint | pop | zorin | elementary | neon | kali)
        log_warn "${OS_NAME} is not Ubuntu. Proceeding, but package versions and defaults may differ."
        ;;
    *)
        case " $OS_LIKE " in
            *" ubuntu "* | *" debian "*) log_warn "Untested distribution '${OS_ID}'. Proceeding anyway." ;;
            *)
                log_error "Unsupported distribution '${OS_ID}'. This installer targets Ubuntu (Debian family)."
                exit 1
                ;;
        esac
        ;;
...
if ! command -v apt-get >/dev/null 2>&1; then
    log_error "apt-get was not found. This installer requires Ubuntu's package manager."
    exit 1
fi
...
RUNNER_PATH="${RUNNER_HOME}/.cargo/bin:${RUNNER_HOME}/.local/bin:/usr/local/bin:/usr/bin:/bin"
...
# Refresh the package index so apt-cache (used for availability filtering) has
# data before the first apt_install call.
apt_refresh
...
if section_enabled base; then
    ...
    apt_ensure_universe
    run_sudo apt-get "${APT_FLAGS[@]}" full-upgrade
    run_sudo apt-get "${APT_FLAGS[@]}" --purge autoremove

    # actions/runner's own runtime dependencies on Debian/Ubuntu. libicu is what
    # the bundled .NET runtime needs for globalization; it is not pulled in by
    # default on a minimal image.
    apt_install libicu-dev libssl-dev zlib1g libkrb5-3

    apt_install \
        build-essential gcc g++ make cmake ninja-build meson pkg-config \
        autoconf automake libtool bison flex gdb strace shellcheck \
        clang libclang-dev llvm lld mold binutils \
        libssl-dev zlib1g-dev libbz2-dev liblzma-dev libreadline-dev libsqlite3-dev \
        libelf-dev libusb-1.0-0-dev libudev-dev libffi-dev \
        libncurses-dev libgmp-dev libmpfr-dev libmpc-dev \
        git git-lfs curl wget rsync jq tree file xxd unzip zip tar gzip xz-utils \
        ca-certificates gnupg \
        procps psmisc lsof util-linux passwd bsdextrautils \
        openssh-client openssh-server \
        ripgrep fd-find bat fzf tmux htop btop socat mosh \
        usbutils pciutils
...
if section_enabled rust; then
    ...
    # sccache comes from APT because the packaged build is already optimized and
...
if section_enabled python; then
    ...
    apt_install python3 python3-pip python3-dev python3-setuptools python3-wheel \
        python3-venv python3-virtualenv pipx uv

    log_info "Note: Ubuntu marks the system Python as externally managed (PEP 668)."
...
if section_enabled arm; then
    ...
    # Ubuntu ships the ARM bare-metal toolchain as gcc-arm-none-eabi (universe);
    # Fedora calls the same toolchain arm-none-eabi-gcc-cs. There is no
    # arm-none-eabi-gdb package, so gdb-multiarch serves as the target debugger.
    apt_install gcc-arm-none-eabi g++-arm-none-eabi binutils-arm-none-eabi \
        libnewlib-arm-none-eabi libstdc++-arm-none-eabi-newlib \
        gdb-multiarch openocd stlink-tools tio

    if command -v arm-none-eabi-gcc >/dev/null 2>&1; then
        log_info "$(arm-none-eabi-gcc --version | head -n 1)"
    else
        log_warn "arm-none-eabi-gcc is not on PATH; check the package list for your Ubuntu release."
    fi
...
if section_enabled qemu; then
    ...
    apt_install qemu-system-arm qemu-user-static qemu-utils
...
if section_enabled containers; then
    ...
    # uidmap provides newuidmap/newgidmap, which rootless Podman needs to write
    # the user namespace mappings. On Ubuntu they are a separate package rather
    # than part of shadow-utils.
    apt_install podman podman-docker buildah skopeo crun conmon \
        fuse-overlayfs slirp4netns passt containers-common uidmap

    # Ubuntu's AppArmor user namespace mediation must be satisfied before rootless
    # containers can start; see the helper for why this is a per-binary profile.
    apparmor_userns_profiles
...
# Managed by runner-install-ubuntu.sh - do not edit by hand.
[Unit]
Description=Rootless Podman API socket for the GitHub Actions runner user
Documentation=https://leftger.github.io/runner-install-ubuntu.sh
After=network-online.target
...
if section_enabled security; then
    ...
    # --- ufw: enabled, default deny inbound, SSH allowed ------------------------
    apt_install ufw
    if command -v ufw >/dev/null 2>&1; then
        if [ "$DRY_RUN" -eq 0 ] && run_sudo ufw status 2>/dev/null | grep -q '^Status: active'; then
            log_info "ufw is already active; leaving the existing ruleset alone."
        else
            # Allow SSH before enabling: switching to the default deny policy with
            # no SSH rule in place would cut off remote access to this machine.
            run_sudo ufw allow OpenSSH
            run_sudo ufw default deny incoming
            run_sudo ufw default allow outgoing
            run_sudo ufw --force enable
            log_info "ufw enabled: inbound denied except OpenSSH."
        fi
    fi
    ...
    # mosh's default range is 60000-61000/udp.
    if [ "$WITH_MOSH" -eq 1 ] && command -v ufw >/dev/null 2>&1; then
        run_sudo ufw allow 60000:61000/udp
...
# Managed by runner-install-ubuntu.sh - kernel and network hardening for a CI node.
...
EOF
    ...
    # --- AppArmor -------------------------------------------------------------
    # Ubuntu's mandatory access control replaces SELinux, so there is nothing to
    # relabel here. What matters for CI is the user namespace mediation that the
    # containers section addresses with per-binary profiles.
    if [ -d /sys/kernel/security/apparmor ]; then
        if userns_restricted; then
            log_info "AppArmor is active and restricts unprivileged user namespaces."
        else
            log_info "AppArmor is active; unprivileged user namespaces are not restricted."
        fi
    else
        log_info "AppArmor is not active on this kernel."
    fi

    # --- optional: lift the AppArmor user namespace restriction ---------------
    if [ "$RELAX_USERNS" -eq 1 ]; then
        if userns_restricted; then
            write_heredoc /etc/sysctl.d/61-gh-runner-userns.conf 0644 root:root <<'EOF'
# Managed by runner-install-ubuntu.sh - requested via --relax-userns.
#
# Permits unprivileged user namespace creation for ALL binaries, so sandbox-style
# test suites (bubblewrap, kind, nsjail, fakeroot) run inside jobs. This removes a
# kernel mitigation Ubuntu enables by default: any local user can then create
# user namespaces, which widens the kernel attack surface.
#
# The default (no --relax-userns) instead installs targeted AppArmor profiles and
# keeps this restriction in place.
kernel.apparmor_restrict_unprivileged_userns = 0
EOF
            run_sudo_quiet sysctl --system || log_warn "Could not apply the userns sysctl."
            log_warn "Unprivileged user namespace creation is now unrestricted system-wide."
        else
            log_info "Kernel already allows unprivileged user namespaces; nothing to relax."
        fi
    fi
...
# Managed by runner-install-ubuntu.sh - CI logs are chatty, keep them bounded.
[Journal]
SystemMaxUse=1G
...
EOF
    ...
    if [ "$WITH_AUTOUPDATE" -eq 1 ]; then
        ...
        # Enabling the periodic run only. The set of permitted origins is left to
        # Ubuntu's own /etc/apt/apt.conf.d/50unattended-upgrades, which already
        # covers the security pocket, so that allowlist is not overwritten here.
        write_heredoc /etc/apt/apt.conf.d/20auto-upgrades 0644 root:root <<'EOF'
// Managed by runner-install-ubuntu.sh - enable periodic unattended upgrades.
APT::Periodic::Update-Package-Lists "1";
APT::Periodic::Unattended-Upgrade "1";
APT::Periodic::Download-Upgradeable-Packages "1";
APT::Periodic::AutocleanInterval "7";
EOF
        run_sudo systemctl enable --now apt-daily.timer apt-daily-upgrade.timer 2>/dev/null || \
            log_warn "Could not enable the apt-daily timers."
        run_sudo systemctl enable --now unattended-upgrades.service 2>/dev/null || true
        log_info "Security updates will be applied automatically by unattended-upgrades."
    fi

    # --- optional fail2ban ----------------------------------------------------
    if [ "$WITH_FAIL2BAN" -eq 1 ]; then
        apt_install fail2ban
        ensure_dir /etc/fail2ban/jail.d 0755
        write_heredoc /etc/fail2ban/jail.d/80-gh-runner-ssh.local 0644 root:root <<'EOF'
# Managed by runner-install-ubuntu.sh - enable the sshd jail.
[sshd]
enabled = true
backend = systemd
EOF
...
# Managed by runner-install-ubuntu.sh - hardening for a headless CI node.
PermitRootLogin no
...
EOF
            if run_sudo sshd -t 2>/dev/null; then
                # Ubuntu names the unit ssh.service; Fedora names it sshd.service.
                run_sudo systemctl reload ssh 2>/dev/null || \
                    run_sudo systemctl reload sshd 2>/dev/null || \
...
# Managed by runner-install-ubuntu.sh - USB debug probe access for the ${RUNNER_USER} group.
...
# Managed by runner-install-ubuntu.sh
...
EOF
    ...
    # --- Ubuntu CLI name shims ------------------------------------------------
    # Ubuntu ships 'bat' as batcat and 'fd' as fdfind to avoid clashing with other
    # packages. Expose the conventional names under the service account's
    # ~/.local/bin, which is on the unit's PATH.
    for pair in "batcat:bat" "fdfind:fd"; do
        shim_src="$(command -v "${pair%%:*}" 2>/dev/null || true)"
        shim_name="${pair##*:}"
        if [ -n "$shim_src" ] && [ ! -e "${RUNNER_HOME}/.local/bin/${shim_name}" ]; then
            ensure_dir "${RUNNER_HOME}/.local/bin" 0755 "${RUNNER_USER}:${RUNNER_USER}"
            run_sudo ln -sfn "$shim_src" "${RUNNER_HOME}/.local/bin/${shim_name}"
            log_info "Exposed ${shim_name} -> ${shim_src}"
        fi
    done
...
if section_enabled runner; then
    ...
    # match them.
    DEFAULT_LABELS="rust,embedded,arm-none-eabi,qemu,thumbv6m,thumbv8m,ubuntu${OS_VERSION}"
    if [ "$WITH_SCCACHE" -eq 1 ]; then
...
# Managed by runner-install-ubuntu.sh - contains a GitHub PAT. Mode 0640 root:${RUNNER_USER}.
...
#!/usr/bin/env bash
# Managed by runner-install-ubuntu.sh - fetch a one-shot JIT config, then run one job.
...
# Managed by runner-install-ubuntu.sh - sccache as the global rustc wrapper.
...
# Managed by runner-install-ubuntu.sh - relaxations required for rootless Podman.
#
...
# Managed by runner-install-ubuntu.sh - CI-friendly cargo defaults with QEMU runners.
...
if section_enabled containers; then
    if [ "$RELAX_USERNS" -eq 1 ]; then
        printf '%s\n' "  • --relax-userns is in effect: unprivileged user namespace creation is"
        printf '%s\n' "    unrestricted system-wide."
    elif userns_restricted; then
        printf '%s\n' "  • AppArmor profiles granting 'userns' were installed for Podman and its"
        printf '%s\n' "    helpers. Suites needing arbitrary user namespaces need --relax-userns."
    fi
fi
...
printf '\n'
printf '%s\n' "To remove everything: sudo ./runner-install-ubuntu.sh --uninstall --token <TOKEN>"
